...
export default class CandidateProfile extends NavigationMixin(LightningElement) {
    ...
    handleClick() {
        const dataToSend = 'Some LWC Data';
        const vfFrame = this.template.querySelector('#vfFrame');
        if (vfFrame && vfFrame.contentWindow) {
          vfFrame.contentWindow.postMessage({ message: 'dataFromLwc', data: dataToSend }, '*');
        } else {
          console.error('Error: VF iframe not loaded or accessible');
        }
      }
    handleSave() {
        this.isLoadingFullScreen = true;
